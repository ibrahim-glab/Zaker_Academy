...
namespace Zaker_Academy.Service.Services
{
    public class AuthorizationService : IAuthorizationService
    {
        ...
        public async Task<ServiceResult> CreateEmailTokenAsync(string username)
        {
            ...
            var res1 = await userManager.ConfirmEmailAsync(user, emailToken);
        ...
        public async Task<ServiceResult> SendResetPasswordAsync(string UserEmail, string callBackUrl)
        {
            ...
            try
            {
                ...
                string body = File.ReadAllText("../Zaker_Academy.Service/Files/ResetPassword.html");
                var token = await userManager.GeneratePasswordResetTokenAsync(user);

                // Send reset password email
                body = body.Replace("token33333", token);
                result = await emailService.sendAsync("Password Reset", body, "jayce.ryan@ethereal.email", UserEmail);
        ...
        public async Task<ServiceResult> SendVerificationEmailAsync(string UserEmail, string callBackUrl)
        {
            ...
            var res = await emailService.sendAsync("Confirmation Email", body, "blanca82@ethereal.email", UserEmail);
